...
## Nutzung im Codex CLI

- Lies diesen Skill mit `open ~/.codex/skills/d-kit/SKILL.md`, bevor du mit einer passenden Aufgabe beginnst.
- Verwende `update_plan`, um Checklistenpunkte aus diesem Skill als ToDos zu verfolgen und abzuhaken.
- Greife bei Bedarf auf die Templates unter `~/.codex/skills/d-kit/references/` zurück (z. B. mit `open ~/.codex/skills/d-kit/references/templates/lastenheft.md`).
- Führe Automation-Scripte aus `~/.codex/skills/d-kit/scripts/` mit `python3` oder `bash` aus, wenn der Skill es verlangt.
- Dokumentiere Learnings und nächste Schritte im Anschluss an jede Phase direkt im Projektkontext.
